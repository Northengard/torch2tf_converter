...
def parse_args(arg_list):
    ...
    parser.add_argument('-w', '--model-weights', type=str, help='torch model weights', default='')
...
if __name__ == '__main__':
    ...
    to_onnx(torch_model, os.path.join(args.input_shape, args.model_name), input_tensor, outputs, ['input'],
            output_names=[f'output{i}' for i in range(len(outputs))])

    model_parser = OnnxInterModel(onnx_path=os.path.join(args.input_shape, args.model_name + '.onnx'),
                                  use_simplify=False)

    tf_model = construct_tf_model(model_parser)
    tf_model_path = os.path.join(args.input_shape, args.model_name + '.pb')
